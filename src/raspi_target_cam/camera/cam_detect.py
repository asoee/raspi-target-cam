...
class WebcamProcessor:
    def __init__(self):
        ...
        self.last_directory_path_save_image_video = "./data/captures"  # Default path
        ...
        self.standard_resolutions = [
            ...
            # [3200, 2400], [3840, 2400],
            [3840, 2560], [4096, 2560],
            [600, 480], [1280,960], [1024,768], [3840, 2448], [3264, 2448], [2592,1944], [2048, 1536],
            # [5120, 2880], [5760, 2880],
            # [4096, 3072],
    ...
    def setup_gui(self):
        ...
        # Format combobox
        ttk.Label(main_frame, text="Video Format:").grid(row=0, column=2, sticky=tk.W, padx=(20, 0))
        self.format_combobox = ttk.Combobox(main_frame, state="readonly", width=15)
        self.format_combobox['values'] = ('MJPG', 'YUY2')
        self.format_combobox.set('YUY2')  # Default to YUY2
        self.format_combobox.bind('<<ComboboxSelected>>', self.on_format_change)
        self.format_combobox.grid(row=0, column=3, sticky=(tk.W, tk.E), padx=(5, 0))
        ...
        self.video_button = ttk.Button(button_frame, text="Start Video Capture", command=self.toggle_video_recording)
    ...
    def on_resolution_change(self, event):
        """Handle resolution combobox selection change"""
        ...
        if selected_resolution:
            ...
            # Apply the new resolution to the video capture
            self.video_capture.set(cv2.CAP_PROP_FRAME_WIDTH, width)
            self.video_capture.set(cv2.CAP_PROP_FRAME_HEIGHT, height)
            
            # Apply the currently selected format
            self.apply_current_format()
            
            # Verify the resolution was set and get camera properties
            ...
            format_code = int(self.video_capture.get(cv2.CAP_PROP_FORMAT))
            fourcc = int(self.video_capture.get(cv2.CAP_PROP_FOURCC))
            
            # Convert fourcc to readable format (handle case where fourcc is 0)
            if fourcc == 0:
                fourcc_str = "Unknown/Default"
            else:
            ...
            self.show_message_on_status_bar(f"Resolution changed to {actual_w}x{actual_h}")
    ...
    def apply_current_format(self):
        """Apply the currently selected format to the video capture"""
        if not self.video_capture or not self.video_capture.isOpened():
            return
            
        selected_format = self.format_combobox.get()
        
        if selected_format == 'MJPG':
            fourcc = cv2.VideoWriter_fourcc(*'MJPG')
        elif selected_format == 'YUY2':
        ...
        # Get and display the actual format that was set
        actual_fourcc = int(self.video_capture.get(cv2.CAP_PROP_FOURCC))
        if actual_fourcc == 0:
            fourcc_str = "Unknown/Default"
        else:
            fourcc_str = "".join([chr((actual_fourcc >> 8 * i) & 0xFF) for i in range(4)]).strip()
        
        self.show_message_on_status_bar(f"Format changed to {selected_format}")
        print(f"Format changed to {selected_format} -> Actual: {fourcc_str} ({actual_fourcc})")
    ...
    def toggle_video_recording(self):
        ...
        if self.is_recording_video:
            if self.video_capture and self.video_capture.isOpened():
                ...
                # Initialize video writer with uncompressed codec
                fourcc = cv2.VideoWriter_fourcc(*'RGBA')  # Uncompressed MJPEG
                self.video_writer = cv2.VideoWriter(video_filename, fourcc, fps, (width, height))
                
                if self.video_writer.isOpened():
                    self.video_button.config(text="Stop Video Capture")
                    self.show_message_on_status_bar(f"Recording video to {video_filename}")
                    print(f"Started video recording: {video_filename}")
                else:
                    self.is_recording_video = False
                    self.show_message_on_status_bar("Failed to start video recording")
                    print("Error: Failed to initialize video writer")
            else:
                self.is_recording_video = False
                self.show_message_on_status_bar("Camera not available for video recording")
        else:
            if self.video_writer:
                self.video_writer.release()
                self.video_writer = None
            self.video_button.config(text="Start Video Capture")
            self.show_message_on_status_bar("Video recording stopped")
            print("Video recording stopped")

    def process_webcam_frames(self, camera_index=0):
            ...
            while True:
                ...
                if self.video_capture.isOpened():
                    ...
                    if ret and frame is not None:
                        ...
                        if self.can_save_image_now:
                            current_time = datetime.now().strftime("%H-%M-%S.%f")[:-3]  # Format: hh-mm-ss.zzz
                            filename = os.path.join(
                                self.last_directory_path_save_image_video,
                                f"{self.number_frames_after_connect}) {current_time}.png"
                            )

                            # Save the original frame (before RGB conversion) as PNG
...
def main():
    """Main entry point for the webcam processor application"""
    app = WebcamProcessor()
    app.run()
